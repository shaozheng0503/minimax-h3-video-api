...
质量档位:
    turbo（默认）: 4 步蒸馏 LoRA 加速，速度约为 standard 的 5 倍
                   - fl2v (文生/图生视频): minimax_h3_fl2v_turbo_4step_v1.0_768p
                   - ref2v (参考图生视频): minimax_h3_ref2v_turbo_4step_v0.1
                   - 标定分辨率 768p（短边 768），更高分辨率可用但质量可能下降
                   - 实测: 720p/5s ≈ 75s；768p/15s ≈ 6-8 分钟（RTX 5090）
    standard:      原生 20 步无加速，画质上限略高
                   - 实测: 720p/5s ≈ 5-7 分钟
...
# 可用的 4 步加速 LoRA（服务端已部署，与本 SDK 默认档位匹配）
TURBO_LORA_FL2V = "minimax_h3_fl2v_turbo_4step_v1.0_768p_comfyui_bf16.safetensors"
...
QUALITY_PRESETS = {
    "turbo": {
        ...
        # 标定分辨率 768p（短边 768，如 1344x768 / 768x1344）
        "lora": TURBO_LORA_FL2V,
        "lora_strength": 1.0,
        "steps": 4,
        "cfg": 1.0,      # 蒸馏模型不使用 CFG 引导
    },
    "standard": {
        # 原生 20 步，无加速 LoRA，画质上限略高
        "lora": None,
        "lora_strength": 0.0,
        "steps": 20,
        "cfg": 7.0,
...
def build_t2v_workflow(
    ...
    if quality not in QUALITY_PRESETS:
        raise ValueError("quality 必须是 'turbo' 或 'standard'，收到: %r" % (quality,))
    ...
    # turbo 档：在 UNet 与 SigmaShift 之间挂 4 步蒸馏 LoRA
    if preset["lora"]:
        workflow["15"] = {
...
def build_ref2v_workflow(
    ...
    if quality == "turbo":
        ...
        if cfg is None:
            cfg = 1.0
        lora_name, lora_strength = TURBO_LORA_REF2V, 1.0
    else:
        if steps is None:
            steps = 20
        if cfg is None:
            cfg = 7.0
        lora_name, lora_strength = None, 0.0
    ...
    workflow = {
        ...
        "8": {"class_type": "KSampler", "inputs": {"seed": seed, "steps": steps, "cfg": cfg, "sampler_name": "euler", "scheduler": "simple", "positive": ["6", 0], "negative": ["7", 0], "latent_image": ["6", 1], "denoise": 1.0}},
    ...
    # 模型链：turbo 挂 ref2v 专用 LoRA，再过 SigmaShift
    if lora_name:
        workflow["15"] = {"class_type": "LoraLoaderModelOnly", "inputs": {"model": ["1", 0], "lora_name": lora_name, "strength_model": lora_strength}}
        workflow["5"] = {"class_type": "MiniMaxH3SigmaShift", "inputs": {"model": ["15", 0], "shift_video": 12.0, "shift_audio": 3.0}}
    else:
        workflow["5"] = {"class_type": "MiniMaxH3SigmaShift", "inputs": {"model": ["1", 0], "shift_video": 12.0, "shift_audio": 3.0}}
    workflow["8"]["inputs"]["model"] = ["5", 0]
...
def generate_video(
    ...
    参数:
        ...
        cfg:         CFG 引导强度（None 则按 quality 档自动）
        seed:        随机种子（None 则随机）
        save_dir:    输出保存目录
