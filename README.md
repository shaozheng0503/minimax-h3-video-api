...
- **已提交的任务从队列中消失** 弹实例在休眠唤醒或自动重启时，会清空执行队列与 history，已排队未执行的任务会直接丢失，客户端表现为"提交成功但长时间无结果"。建议：关键任务使用异步模式（回调驱动，便于发现丢失并重提）；轮询模式下若发现任务 ID 从 `/history` 消失且队列为空，应立即重提而非继续等待。另外，实例重启也会清空已上传的输入图片——图生视频/参考生视频遇到 `Invalid image file` 报错时，重新上传图片再提交即可。
