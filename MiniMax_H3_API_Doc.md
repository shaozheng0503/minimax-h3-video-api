...
| 参数 | 默认值 | 推荐范围 | 说明 |
|------|--------|---------|------|
| steps | 20 | 15-30 | 采样步数，越多质量越好但越慢 |
| cfg | 7.0 | 5.0-9.0 | CFG 引导强度 |
| sampler_name | euler | euler | 采样器，推荐 euler |
| scheduler | simple | simple | 调度器，推荐 simple |
| seed | 随机 | 0 ~ 2^32-1 | 随机种子 |
...
冷启动耗时约 5-10 分钟，之后单次生成降至 1-2 分钟。

**执行缓存（Execution Cache）**：ComfyUI 会自动缓存已执行节点的结果。如果连续提交相同参数的工作流（相同 seed、prompt、尺寸），后续请求可能直接命中缓存，返回时间可降至 **2-3 秒**。

> 实测：首次提交 720p/124帧 工作流耗时 ~330 秒（含冷启动），后续相同参数提交仅 2.8 秒（缓存命中）。更换 seed 或 prompt 后恢复正常推理耗时。
...
> 实测数据：720p / 124 帧 / 20 steps，推理耗时约 424 秒（7 分钟），响应体约 2.3MB。
